...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($newStatus) {
        ...
        $query = "UPDATE sensor_data SET Pintu = ? WHERE id = (SELECT MAX(id) FROM sensor_data)";
        ...
        if ($stmt->execute()) {
            ...
            echo json_encode(['error' => 'Gagal memperbarui data pintu']);
        }
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $query = "SELECT Pintu FROM sensor_data WHERE id = (SELECT MAX(id) FROM sensor_data)";
    $result = $koneksi->query($query);

    if ($result && $row = $result->fetch_assoc()) {
        echo json_encode(['status' => $row['Pintu']]);
    } else {
        echo json_encode(['error' => 'Gagal membaca data pintu']);
    }
    exit;
}
